...
import { STATUS_COLORS, AGENT_COLORS, DEFAULT_PORT, DEFAULT_CLIENT_PORT } from "../constants.js";
...
describe("STATUS_COLORS", () => {
  it("has a color for every agent status", () => {
    ...
    for (const status of statuses) {
      ...
      expect(STATUS_COLORS[status as keyof typeof STATUS_COLORS]).toMatch(/^#[0-9a-fA-F]{6}$/);
    }
  });
