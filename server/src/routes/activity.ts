    ...
    for (const cycleNum of cycles) {
      ...
      if (researcherEntry && agentMap.researcher.length < limit) {
        ...
        agentMap.researcher.push({
          ...
          timestamp: ts,
          detailedTask:
            artifactPreviews[artifactKey] ||
            `Research phase for ${researcherEntry.task.replace(/-/g, ' ')}`,
          verdict: '',
          resultLength: 0,
        })
      }
      ...
      if (coderEntry.length > 0 && agentMap.qa.length < limit) {
        ...
        const rejected = coderEntry.filter((e) =>
          e.description.includes('rejected'),
        )
        const approved = coderEntry.find((e) =>
          e.description.includes('approved'),
        )
        let qaDesc: string
        ...
        agentMap.qa.push({
          ...
          verdict: qaVerdict.includes('approved')
            ? 'PASS'
            : qaVerdict.includes('rejected')
              ? 'FAIL'
              : '',
          resultLength: 0,
        })
