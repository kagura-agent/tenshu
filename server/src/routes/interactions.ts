...
interactions.get("/", async (c) => {
  try {
    ...
    // Parse results
    const results: Array<{ cycle: number; agent: string; task: string; score: number; status: string }> = [];
    for (const line of lines) {
      const [, cycle, task, agent, score, status] = line.split("\t");
      results.push({ cycle: Number(cycle), agent, task, score: Number(score), status });
    }

    // Build node stats
    const agentStats: Record<string, { tasks: number; totalScore: number; scoreCount: number }> = {};
    for (const r of results) {
      if (!agentStats[r.agent]) agentStats[r.agent] = { tasks: 0, totalScore: 0, scoreCount: 0 };
    ...
    const AGENT_NAMES: Record<string, string> = {
      researcher: "Senku", coder: "Bulma", qa: "Vegeta", planner: "Erwin", comms: "Jet",
    };
    ...
    const nodes: AgentNode[] = Object.entries(agentStats).map(([id, s]) => ({
      ...
      avgScore: s.scoreCount > 0 ? Math.round((s.totalScore / s.scoreCount) * 10) / 10 : 0,
    }));
    ...
    const edgeCounts: Record<string, { count: number; scores: number[]; tasks: Set<string> }> = {};
    ...
    for (const [cycleNum, agents] of cycleMap) {
      ...
      for (let i = 0; i < ordered.length - 1; i++) {
        ...
        if (!edgeCounts[key]) edgeCounts[key] = { count: 0, scores: [], tasks: new Set() };
    ...
    const edges: DelegationEdge[] = Object.entries(edgeCounts).map(([key, data]) => {
