...
function createMockWS(): WSContext & { sentMessages: string[] } {
  ...
  const ws = new WSContext({
    send(data: string | ArrayBuffer | Uint8Array) {
      sentMessages.push(typeof data === "string" ? data : new TextDecoder().decode(data as Uint8Array));
    },
    close() {},
    readyState: 1,
  });
...
describe("WebSocket handler", () => {
  ...
  describe("connection lifecycle", () => {
    it("handles open → message → close cycle", async () => {
      const { addClient, removeClient, broadcast, getClientCount } = await loadHandler();
      const ws = createMockWS();
