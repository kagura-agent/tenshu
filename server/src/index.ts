...
try {
  ...
  watchConfig(openclawDir, (config) => {
    console.log(
      `[tenshu] openclaw.json reloaded — ${config.agents.length} agents`,
    )
  })
} catch (e) {
  console.warn('[tenshu] Could not load openclaw.json:', (e as Error).message)
  console.warn(
    '[tenshu] Starting in demo mode — client will use simulated data.',
  )
  demoMode = true
}
...
console.log(
  `天守 Tenshu server running on http://localhost:${port}${modeLabel}`,
)
