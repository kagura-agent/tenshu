...
const SOUND_MAP: Record<ThemeMode, Record<SoundEvent, () => void>> = {
  warroom: {
    ...
    "cycle-complete": () => { playTaiko(); setTimeout(playBamboo, 300); },
  ...
  deck: {
    ...
    "cycle-complete": () => { playSynthBeep(); setTimeout(playDigitalWhoosh, 200); },
  ...
  garden: {
    ...
    "cycle-complete": () => { playWindChime(); setTimeout(playWaterDrop, 500); },
...
export function useSoundOnStatusChange(agentStatuses: Record<string, string>) {
  ...
  useEffect(() => {
    ...
    for (const [id, status] of Object.entries(agentStatuses)) {
      const old = prev[id];
      if (old && old !== status) {
        if (status === "working" || status === "thinking") play("status-working");
