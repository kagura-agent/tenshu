...
export function computePowerLevel(entries: CycleEntry[]): PowerLevel {
  if (entries.length === 0) {
    return { xp: 0, level: 0, levelName: "Genin", nextLevelXp: 500, progress: 0, powerLevel: 0 };
  }
  ...
  const nextThreshold = levelIdx < LEVELS.length - 1 ? LEVELS[levelIdx + 1].xp : LEVELS[levelIdx].xp;
  ...
  const keepRate = entries.filter((e) => e.status === "keep").length / entries.length;
  const powerLevel = Math.round(avgScore * entries.length * (0.5 + keepRate * 0.5));
