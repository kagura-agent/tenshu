...
const ACHIEVEMENTS: AchievementDef[] = [
  ...
  },
  {
    ...
    check: (rows) => {
      let streak = 0;
      for (const r of rows) {
        if (r.status === "keep") { streak++; if (streak >= 3) return r.timestamp; }
        else streak = 0;
      }
      return null;
    },
  },
  {
    ...
    check: (rows) => {
      let streak = 0;
      for (const r of rows) {
        if (r.status === "keep") { streak++; if (streak >= 5) return r.timestamp; }
        else streak = 0;
      }
      return null;
    },
  },
  {
    ...
    check: (rows) => {
      let streak = 0;
      for (const r of rows) {
        if (r.status === "keep") { streak++; if (streak >= 10) return r.timestamp; }
        else streak = 0;
      }
      return null;
    },
  },
  {
    ...
    check: (rows) => {
      for (let i = 1; i < rows.length; i++) {
        if (rows[i].status === "keep" && rows[i - 1].status === "discard") return rows[i].timestamp;
      }
      return null;
    },
  },
