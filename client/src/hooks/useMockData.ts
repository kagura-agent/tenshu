...
export function useMockCurrentCycle() {
  ...
  const current: CurrentCycle = {
    ...
    recentLines: DEMO_LOG_LINES.slice(Math.max(0, lineIndex - 3), lineIndex + 1),
  };
...
const DEMO_STATUSES: ResultRow["status"][] = ["keep", "keep", "keep", "discard", "keep", "crash", "keep", "skip"];
...
function generateResults(count: number): ResultRow[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    rows.push({
      ...
      task: pick(DEMO_CYCLE_TASKS),
      agent,
      score: status === "crash" ? 0 : Math.round((4 + Math.random() * 6) * 10) / 10,
      status,
      description: pick(DEMO_TASKS),
    });
  }
