...
function generateDemoData(): { nodes: AgentNode[]; edges: DelegationEdge[] } {
  const nodes: AgentNode[] = [
    { id: "planner", name: "Erwin", role: "planner", tasksCompleted: 170, avgScore: 6.2 },
    { id: "researcher", name: "Senku", role: "researcher", tasksCompleted: 168, avgScore: 6.1 },
    { id: "coder", name: "Bulma", role: "coder", tasksCompleted: 165, avgScore: 6.6 },
    { id: "qa", name: "Vegeta", role: "qa", tasksCompleted: 160, avgScore: 6.4 },
    { id: "comms", name: "Jet", role: "comms", tasksCompleted: 50, avgScore: 7.0 },
  ];
  const edges: DelegationEdge[] = [
    { from: "planner", to: "researcher", count: 168, avgScore: 6.3, tasks: ["ai-self-improvement", "tool-building"] },
    { from: "researcher", to: "coder", count: 165, avgScore: 6.5, tasks: ["ai-self-improvement", "tool-building"] },
    { from: "coder", to: "qa", count: 160, avgScore: 6.6, tasks: ["ai-self-improvement", "tool-building"] },
    { from: "planner", to: "comms", count: 30, avgScore: 7.0, tasks: ["frontier-scan"] },
    { from: "qa", to: "coder", count: 45, avgScore: 5.2, tasks: ["tool-building"] }, // retries
  ];
  return { nodes, edges };
}
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (const node of simNodes) {
        ...
        // Score badge
        ctx.fillStyle = node.avgScore >= 7 ? "#22c55e" : node.avgScore >= 5 ? color : "#ef4444";
...
export function Interactions() {
  ...
  const avgEdgeScore = edges.length > 0
    ? Math.round((edges.reduce((s, e) => s + e.avgScore * e.count, 0) / totalDelegations) * 10) / 10
    : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-3">
        <ThemedCard>
          ...
          <p className="text-2xl font-bold" style={{ color: accent }}>{nodes.length}</p>
        </ThemedCard>
        <ThemedCard>
          <p className="text-xs text-zinc-500 uppercase tracking-wider">Delegations</p>
        ...
        <ThemedCard>
          ...
          <p className="text-2xl font-bold" style={{ color: avgEdgeScore >= 7 ? "#22c55e" : accent }}>
            {avgEdgeScore}
          </p>
        </ThemedCard>
      </div>
      ...
      <ThemedCard>
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs text-zinc-500 uppercase tracking-wide">Agent Delegation Flow</p>
        ...
        {nodes.length > 0 ? (
          ...
          <p className="text-zinc-500 text-sm text-center py-16">No interaction data yet.</p>
        )}
      </ThemedCard>
      ...
      {edges.length > 0 && (
        <ThemedCard>
          <p className="text-xs text-zinc-500 uppercase tracking-wide mb-3">Delegation Details</p>
          <div className="space-y-2">
            {edges.sort((a, b) => b.count - a.count).map((edge) => (
              <div key={`${edge.from}-${edge.to}`} className="flex items-center gap-3 text-sm">
                <span style={{ color: ROLE_COLORS[edge.from] }} className="font-medium w-20">{edge.from}</span>
                <span className="text-zinc-600">→</span>
                <span style={{ color: ROLE_COLORS[edge.to] }} className="font-medium w-20">{edge.to}</span>
                <span className="text-zinc-400 font-mono text-xs">{edge.count}x</span>
                <span className="text-zinc-500 font-mono text-xs">avg {edge.avgScore}/10</span>
